...
void LevelEditorSystem::Init(){
    ...
    // [T] 存储地图测试
    coordinator.AddEventListener(EVENT::EVENT_ON_KEYBOARD_CLICKED_I, [this](const Event& event){
        OnKeybardClicked_I(event);
    });

    // [T] 扩容地图测试
    coordinator.AddEventListener(EVENT::EVENT_ON_KEYBOARD_CLICKED_U, [this](const Event& event){
        OnKeybardClicked_U(event);
    });
}
...
void LevelEditorSystem::OnMouseClicked_Left(const Event &event)
{
    if (coordinator.m_State == GAME_STATE::EDITOR)
    {
        ...
        PutTile(m_MouseX, m_MouseY, tileX, tileY, m_CameraOffset, offset, 8);
    }
}
...
void LevelEditorSystem::SetTile(int x, int y, int type)
{
    ...
    else
    {
        ...
        // coordinator.m_Tilemap.m_UpdatedPairs.insert(make_pair(x, y));
        // for(const auto& tile : coordinator.m_Tilemap.m_UpdatedPairs){
        //     cout << "Inserting data to the set: " << tile.first << ", " << tile.second << endl;
        // }
